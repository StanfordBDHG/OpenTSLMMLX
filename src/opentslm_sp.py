...
=======
        """Load trained encoder/projector/LoRA weights from a safetensors checkpoint."""
        checkpoint = mx.load(path)

        if "encoder.pos_embed" not in checkpoint:
            raise ValueError(
                "Checkpoint is missing required key 'encoder.pos_embed'. "
                "Expected pre-converted .safetensors format."
            )

        max_patches = checkpoint["encoder.pos_embed"].shape[1]
        self.encoder = TransformerCNNEncoder(max_patches=max_patches)
        ...
        if not encoder_weights or not projector_weights:
            raise ValueError(
